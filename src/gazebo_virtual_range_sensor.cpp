  ...
  , _ground_plane(Eigen::Vector3d(0, 0, -_sensor_transform.getOrigin().z()), Eigen::Vector3d::UnitZ())
{
  ...
  robot::algorithm::rotate_plane(_ground_plane, robot::message::from_ros(_sensor_transform.getRotation()).conjugate());
}

void SensorVirtualRange::initialize(const robot::SensorRange::Parameter& parameter)
{
  (void)parameter;
}
...
void SensorVirtualRange::processPointCloudMeasurement(sensor_msgs::msg::PointCloud2& point_cloud)
{
  ...
  for (sensor_msgs::PointCloud2ConstIterator<float> point(point_cloud, "x"); point != point.end(); ++point) {  
    ...
    if (std::isnan(p.z()) || p.z() == 0.0f || robot::algorithm::is_above_plane(_ground_plane, p) == false) {
